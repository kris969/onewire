...
typedef struct  {
	 ...
	 uint8_t 	charger_activation_mode : 1 ;// [BIT3] Indicate whether the battery charger entered into activation mode, 0:Did not enter the cell activation mode; 1:Has entered the cell activation mode
...
float AXP209_internal_temperature(void) ;
float VBUS_voltage(void) ;
float VBUS_current(void) ;

int i2c_init(char *i2c_bus, unsigned char address) ;

bool battery_is_connected(void) ;
